...
export class ReservasService {
    ...
    constructor(
        ...
        this.MULTA_VALOR_FIXO = parseFloat(this.configService.get('MULTA_VALOR_FIXO', '10'));
        this.MULTA_PERCENTUAL_DIA = parseFloat(this.configService.get('MULTA_PERCENTUAL_DIA', '0.05'));
    }
    ...
    async remove(id: string): Promise<void> {
        ...
        if (reserva.status === ReservaStatus.ATIVA || reserva.status === ReservaStatus.ATRASADA) {
            await this.livrosService.marcarComoDisponivel(reserva.livroId._id?.toString() || reserva.livroId.toString());
        }
    ...
    async devolver(id: string): Promise<Reserva> {
        ...
        // Atualizar reserva
        const updated = await this.reservaModel.findByIdAndUpdate(
            id,
            {
    ...
    private calcularMultaSeAtrasada(reserva: any): any {
        ...
        if (hoje > dataPrevista) {
            ...
            const valorTotal = this.MULTA_VALOR_FIXO + (this.MULTA_VALOR_FIXO * this.MULTA_PERCENTUAL_DIA * diasAtraso);
    ...
    calcularMulta(diasAtraso: number): number {
        ...
        return this.MULTA_VALOR_FIXO + (this.MULTA_VALOR_FIXO * this.MULTA_PERCENTUAL_DIA * diasAtraso);
    }
}
